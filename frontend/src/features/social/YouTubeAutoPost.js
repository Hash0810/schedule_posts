...
function YouTubeAutoPost() {
  ...
  useEffect(() => {
    fetch("/api/socialauth/status/youtube")
      .then(res => res.json())
  ...
  const handleConnect = async () => {
    const res = await fetch("/api/socialauth/connect/youtube");
  ...
  return (
    <div className="max-w-lg mx-auto mt-10 p-8 bg-white rounded-3xl shadow-2xl border border-blue-100 animate-fade-in">
      <div className="flex items-center gap-3 mb-6">
        <YouTubeIcon />
        <h2 className="text-2xl font-extrabold text-blue-900 tracking-tight">YouTube Auto Post</h2>
      </div>
      {!connected && (
        <div className="mb-6 p-4 bg-yellow-50 border border-yellow-200 rounded-xl flex items-center gap-3">
          <span className="text-yellow-600 font-bold">⚠️</span>
          <div className="flex-1">
            <p className="text-yellow-800 mb-2">Connect your YouTube account to schedule videos</p>
      ...
      {connected && (
        <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-xl flex items-center gap-3">
          ...
          <div className="flex-1 text-green-800 font-semibold">YouTube connected successfully!</div>
        </div>
      )}
      <form onSubmit={handleSchedule} className="space-y-6">
        ...
        <button 
          ...
          disabled={!connected}
        >
          {connected ? 'Schedule Video' : 'Connect YouTube First'}
        </button>
      </form>
      {message && (
...
export default YouTubeAutoPost;

function YouTubeIcon() {
  return (
    <svg width="36" height="36" fill="none" viewBox="0 0 36 36"><circle cx="18" cy="18" r="18" fill="#FF0000"/><polygon points="15,13 24,18 15,23" fill="#fff"/></svg>
  );
}
